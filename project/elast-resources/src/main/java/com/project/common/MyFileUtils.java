...
public class MyFileUtils {
	...
	public static String getCurrentClassPath() {
		String path = MyFileUtils.class.getClassLoader().getResource("")
				.getPath().substring(1).replace("%20", " ").replace("/", "\\");
		return path;
	}

	// 在指定路径生成文件/并写入字符串
	public static void writeFile(String target, String fileName,
	...
	// 拷贝文件到指定路径/设置文件名
	public static void copyFileSetName(String source, String target,
			String newFileName, int mode) {
		// mode=0时/不拷贝已存在的文件
		if (mode == 0) {
